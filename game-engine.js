...
const ING_DEFS = [
  ...
  {id:'tofu',name:'豆腐',cat:'protein',catL:'たんぱく質',eff:'自分はごみ箱から「味噌」のカードを1枚選び、手札に加える。'},
  ...
  {id:'miso',name:'味噌',cat:'protein',catL:'たんぱく質',eff:'自分は手札の「たんぱく質」のカードを1枚選び、ごみ箱に送ってもよい。そうしたなら、自分は食材山札からカードを1枚引く。ごみ箱に送ったカードが「豆腐」なら、自分はさらに食材山札からカードを1枚引く。'},
  {id:'abura',name:'油揚げ',cat:'fat',catL:'脂質',eff:'手札のレシピカードを1枚公開する。そのカードをレシピ山札に戻し、レシピ山札から1枚引く。'},
  {id:'butter',name:'バター',cat:'fat',catL:'脂質',eff:'このカードに【満腹バフ：1】を付与する。'},
  ...
  {id:'tomato',name:'トマト',cat:'vitamin',catL:'ビタミン',eff:'自分の食材ゾーンのカードが2枚以下なら、自分は食材山札からカードを2枚引く。'},
...
const REC_DEFS = [
  ...
  {id:'yasaiSalad',name:'温野菜サラダ',sat:5,req:['carrot','potato','broccoli'],eff:'手札を1枚ごみ箱に送ってもよい。そうしたなら、ごみ箱から食材カードを1枚選び手札に加える。'},
  {id:'omelet',name:'オムレツ',sat:5,req:['egg','milk','butter'],eff:'食材山札から2枚引く。その後、手札を1枚ごみ箱に送る。'},
  {id:'hiyakko',name:'冷やっこ',sat:4,req:['tofu','ginger','negi'],eff:'食材山札から1枚引く。その後、自分の満腹度-2。'},
  {id:'horenSote',name:'ほうれん草のソテー',sat:5,req:['pork','butter','spinach'],eff:'自分と相手はランダムな手札1枚を山札に戻す。'},
  {id:'namuL',name:'野菜のナムル',sat:5,req:['spinach','carrot','moyashi'],eff:'自分と相手はランダムな手札1枚を山札に戻す。その後、自分と相手は食材山札からカードを1枚引く。'},
  ...
  {id:'peperoncino',name:'ペペロンチーノ',sat:5,req:['flour','garlic','chili'],eff:'相手は手札を全て公開する。'},
  ...
  {id:'curryBread',name:'カレーパン',sat:6,req:['flour','curryroux','egg','butter'],eff:'自分は食材山札から食材カードを1枚選ぶ。その後、そのカードを食材ゾーンに出す。'},
  ...
  {id:'caesarSalad',name:'シーザーサラダ',sat:7,req:['flour','pork','lettuce','cheese'],eff:'レシピ山札からレシピカードを1枚選び手札に加える。その後、自分の満腹度+1。'},
  ...
  {id:'hamburg',name:'ハンバーグ',sat:8,req:['beef','pork','egg','onion'],eff:'食材山札から4枚引く。次の自分のターン中、料理を提供できない。'},
  ...
  {id:'carbonara',name:'カルボナーラ',sat:8,req:['flour','egg','pork','milk','cheese'],eff:'手札を全て山札に戻す。その後、食材山札から5枚引き、レシピ山札から2枚引く。'},
  {id:'butajiru',name:'豚汁',sat:6,req:['pork','miso','carrot','konjac','daikon'],eff:'食材山札から2枚引く。その後、自分の満腹度-3。'},
  {id:'nikujaga',name:'肉じゃが',sat:9,req:['potato','beef','carrot','onion','konjac'],eff:'ごみ箱からレシピカードを1枚選び手札に加える。その後、手札を1枚ごみ箱に送る。'},
  {id:'okosamLunch',name:'お子様ランチ',sat:12,req:['rice','beef','egg','carrot','broccoli','tomato'],eff:'レシピ山札から3枚引く。その後、手札を2枚山札に戻す。'},
  ...
  {id:'meatSauce',name:'ミートソースパスタ',sat:10,req:['flour','beef','tomato','carrot','onion','garlic'],eff:'手札を全て山札に戻す。その後、食材山札からX枚引く（X=相手の食材ゾーンの枚数）。'},
  ...
  {id:'curryRice',name:'カレーライス',sat:11,req:['rice','potato','beef','curryroux','carrot','onion'],eff:'食材山札から2枚引き、そのカードを食材ゾーンに出す。'},
  {id:'sukiyaki',name:'すき焼き',sat:13,req:['konjac','beef','tofu','negi','carrot','shiitake'],eff:'相手の手札が7枚以上なら、相手の満腹度+2。そうでないなら、自分の満腹度+2。'},
];
...
function execBlock(G, pi, blk, cardId) {
  ...
  switch (blk.type) {
    ...
    case 'condLastTrash': {
      const lastTrash = p.trash.length > 0 ? p.trash[p.trash.length - 1] : null;
      if (!lastTrash || lastTrash.id !== blk.matchId) return addLog(G, `P${pi + 1}: 条件不成立（豆腐でない）`);
      return runBlocks(G, pi, blk.then, card);
    }
    case 'discardHand': {
    ...
    case 'markIngZone': {
      if (!p.ingZone.length) return addLog(G, '食材ゾーンが空');
      return mkPending(G, { type: 'markIngZone', pi, buffN: blk.n || 3 });
    }
    case 'markSelf': {
      // 起動したカード自身に満腹バフを付与
      if (!card) return G;
      const buffVal = blk.n || 1;
      const iz = p.ingZone.map(z =>
        z._uid === card._uid ? { ...z, _satBuff: (z._satBuff || 0) + buffVal } : z
      );
      return addLog(ovP(G, pi, { ingZone: iz }), `P${pi + 1}: ${card.name}に満腹バフ+${buffVal}付与`);
    }
    default: return G;
  }
}
...
function matchFilter(c, key) {
  ...
  if (key === 'miso_only') return c.id === 'miso' && !c._isRec;
...
function resolveChoice(G, pi, pd, chosen) {
  ...
  if (pd.type === 'markIngZone') {
    ...
    return addLog(ovP(G, pi, { ingZone: p.ingZone.map(c => c._uid === chosen[0] ? { ...c, _satBuff: pd.buffN || 3 } : c) }), `P${pi + 1}: ${target.name}に【満腹バフ：${pd.buffN || 3}】を付与`);
  }
  return G;
}
...
const ING_BLOCKS = {
  ...
  tofu: [{ type: 'recoverTrash', filterKey: 'miso_only' }],
  chicken: [{ type: 'drawToZone', n: 1 }],
  pork: [{ type: 'selectIngZoneMulti', n: 2 }],
  yellowtail: [{ type: 'drawIng', n: 1 }, { type: 'retHand', n: 1 }],
  miso: [{ type: 'discardHand', n: 1, optional: true, filterKey: 'protein' }, { type: 'ifPrev' }, { type: 'drawIng', n: 1 }, { type: 'condLastTrash', matchId: 'tofu', then: [{ type: 'drawIng', n: 1 }] }],
  ...
  tomato: [{ type: 'condIngZone', op: 'le', n: 2, then: [{ type: 'drawIng', n: 2 }] }],
...
const REC_BLOCKS = {
  ...
  hamburg: [{ type: 'drawIng', n: 4 }, { type: 'setDebuff', key: 'noServe', val: 1 }],
  curryUdon: [{ type: 'drawToZone', n: 2 }],
  okonomiyaki: [], yasaiItame: [],
  carbonara: [{ type: 'retAllHand', ing: 5, rec: 2 }],
  butajiru: [{ type: 'drawIng', n: 2 }, { type: 'redSelfSat', n: 3 }],
